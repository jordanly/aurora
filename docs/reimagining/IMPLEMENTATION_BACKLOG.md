...
| P5 follow-up / JAVA-11 | In progress: [Java 25 minimum and whole-repository convergence](JAVA25_BASELINE.md); [step 2 build status](JAVA11_BUILD_STATUS.md). Step 2 implementation is complete, pending fresh source packaging and physical qualification. | JAVA-08, qualified native baseline | Root `./gradlew` uses the pinned Java 25+/Gradle 9 graph for `:aurora-native-scheduler` and `:protocol`; obsolete Java 8/Gradle 4 bootstrap, buildSrc and current-source smoke route are removed; retained lab utilities pass their checks. Fresh source packaging and physical qualification remain pending. Steps 3 (legacy source retirement) and 4 (SQL idioms) remain. |
